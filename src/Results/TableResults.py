...
class TableResults:
    ...
    def build_dataframe(
        ...
        dedup_cols = ["Dataset", "Model", "Scenario", "Threshold", "Decision"]
        df_final = df_final.drop_duplicates(subset=dedup_cols, keep="last")
        sort_cols = ["Dataset", "Model", "Scenario", "Threshold", "Decision"]
        return df_final.sort_values(sort_cols).reset_index(drop=True)

    def _highlight_indices(self, df: pd.DataFrame) -> dict:
        highlights = {"global_best": set(), "local_best": set(), "global_worst": set()}
        if df.empty or "F1-Score" not in df.columns:
            return highlights
        best_value = df["F1-Score"].max()
        worst_value = df["F1-Score"].min()
        if pd.notnull(best_value):
            highlights["global_best"].update(df[df["F1-Score"] == best_value].index.tolist())
        if pd.notnull(worst_value):
            highlights["global_worst"].update(df[df["F1-Score"] == worst_value].index.tolist())
        if "Dataset" in df.columns:
            for _, group_df in df.groupby("Dataset", dropna=False):
                local_value = group_df["F1-Score"].max()
                if pd.notnull(local_value):
                    highlights["local_best"].update(group_df[group_df["F1-Score"] == local_value].index.tolist())
        highlights["local_best"] = highlights["local_best"] - highlights["global_best"] - highlights["global_worst"]
        return highlights
    ...
    def _build_cover_page(
        ...
        story = [
            ...
            Paragraph("<b>Fixed / Raw:</b> baseline decision rule. Each anomaly score is compared directly with the established threshold. If the score is greater than the threshold, the instance is classified as malicious; otherwise, it is classified as benign. In the table, this direct decision can appear as <b>raw</b>.", text_style),
            Spacer(1, 0.15 * cm),
            Paragraph("<b>Moving average:</b> temporal decision rule based on a causal smoothing of recent anomaly scores instead of only the current score. In this study, windows of 3 and 5 recent scores were evaluated. In the table, these configurations can appear as <b>moving_average_w3</b> and <b>moving_average_w5</b>.", text_style),
            Spacer(1, 0.15 * cm),
            Paragraph("<b>Temporal persistence:</b> temporal decision rule in which the anomaly condition must occur repeatedly within a recent window before the instance is classified as malicious. In this study, the 2-of-3 and 3-of-5 configurations were evaluated. In the table, these configurations can appear as <b>persistence_2_of_3</b> and <b>persistence_3_of_5</b>.", text_style),
            Spacer(1, 0.35 * cm),
        ...
        legend_data = [
            [
                Paragraph("Global best result", legend_style),
                Paragraph("Local best result by dataset", legend_style),
                Paragraph("Global worst result", legend_style),
            ],
            [
                Paragraph("Highest F1-Score among all evaluated scenarios.", text_style),
                Paragraph("Highest F1-Score within each dataset.", text_style),
                Paragraph("Lowest F1-Score among all evaluated scenarios.", text_style),
            ],
        ]

        legend_table = Table(legend_data, colWidths=[6.2 * cm, 7.0 * cm, 6.2 * cm])
        ...
        story.append(Paragraph("The Exec_ID is used only internally to select the most recent execution when no specific identifier is provided. It is not displayed in the final table. The table itself starts on the next page to keep the legend and methodological notes centralized.", text_style))
        return story

    def save_pdf(
        self,
        df: pd.DataFrame,
        title: str = "Complete Results Table",
        ...
        final_filename = filename or f"{self._clean_filename(title)}.pdf"
        pdf_path = os.path.join(final_dir, final_filename)
        ...
        for start in range(0, len(df_pdf), rows_per_page):
            ...
            for local_idx, original_idx in enumerate(page_df.index, start=1):
                if original_idx in highlights["global_best"]:
