...
class Plots:
    ...
    def plot_score(self, results, attack_regions, title="Análise de Scores", threshold=0.5):
        ...
        for i, (name, data) in enumerate(results.items()):
            if 'scores' not in data: continue
        ...
        leg = ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=len(results) + 2, fontsize=12, frameon=False)
    ...
    def plot_metrics(self, results, attack_regions=None, title="Métricas", window_size=1000, target_class=None):
        ...
        for i, (name, data) in enumerate(results.items()):
            ...
            f1_data = data.get('f1', data.get('f1_score', [])) 
            def clean(d_list): return [0.0 if (v is None or np.isnan(v)) else v for v in d_list]

            ax1.plot(x_axis, clean(f1_data), label=f'{name}', color=color, linewidth=2.5, zorder=3, marker='o', markersize=5)
        ...
        # Legenda global na figura, evitando coordenadas relativas exageradas
        ...
        # Gerenciamento manual de espaço
